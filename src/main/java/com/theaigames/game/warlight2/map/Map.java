...
public class Map {
    ...
    private String getNeighborsAsList(Region region)
    {
	StringBuilder neighborsString = new StringBuilder();
	neighborsString.append("[");
	String sep = "";
	for (Region nb : region.getNeighbors()) {
	    if (nb.getId() > region.getId()) {
		neighborsString.append(sep + "\"" + nb.getId() + "\"");
		sep = ",";
	    }
	}
	neighborsString.append("]");
    ...
    public String getMapAsDictString()
    {
	StringBuilder mapString = new StringBuilder();
	mapString.append("{\"Regions\":[");
	String sep = "";
	for (Region region : regions) {
	    mapString.append(sep + "{\"id\": \"" + region.getId() + "\"" +
			     ",\"superRegion\": \"" + region.getSuperRegion().getId() + "\"" +
			     ",\"neighbors\":" + getNeighborsAsList(region) +
			     "}");
	    sep = ",";
	}
	sep = "";
	mapString.append("],\"SuperRegions\":[");
	for (SuperRegion superRegion : superRegions) {
	    mapString.append(sep + "{\"id\": \"" + superRegion.getId() + "\"" +
			     ", \"bonus\": \"" + superRegion.getArmiesReward() +
			     "\"}");
	    sep = ",";
	}
	mapString.append("]}");
